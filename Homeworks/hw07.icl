...
gimmeMoney accountArray b
= {{x & balance = ( x.balance + b)} \\ x <-: accountArray}
...
isRichest :: {Account} Account Int -> String
isRichest accountArray richAcc i
| i == (length [e\\e<-:accountArray]) = richAcc.owner.name
| accountArray.[i].balance > richAcc.balance = isRichest accountArray accountArray.[i] (i+1)
= isRichest accountArray richAcc (i+1)


soRich :: {Account} -> String
soRich accountArray
=  isRichest selectedArray selectedArray.[0] 1
where selectedArray = {x\\x<-:accountArray|x.owner.isDead == False}
...
query (f,startYear,endYear) accountArray
= {e \\ e<-:accountArray|(f e)&&(e.dateCreated.year <= endYear )&&(e.dateCreated.year >= startYear )}
